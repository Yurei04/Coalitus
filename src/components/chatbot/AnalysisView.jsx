...
import { Brain, HeartPulse, BookOpen, GraduationCap, Wifi, WifiOff, Loader2 } from "lucide-react";
...
function ModelPanel({ model, scores, enabled, isAnalysing }) {
    ...
    >
      ...
      <div className="flex items-center justify-between px-4 py-3"
        ...
        <div className="flex items-center gap-1.5">
          {isAnalysing ? (
            <Loader2 size={12} className="animate-spin" style={{ color: model.color }} />
          ) : enabled ? (
      ...
      <div className="px-4 py-4">
        {!enabled ? (
          ...
          <div className="flex flex-col gap-2 py-1">
            {[0.75, 0.55, 0.38, 0.22].map((w, i) => (
              <div key={i} className="h-5 rounded-lg animate-pulse"
                style={{ width: `${w * 100}%`, background: `${model.color}12` }} />
            ))}
          </div>
...
export function AnalysisView({ orderedModels, enabledModels }) {
  const { modelResults, isAnalysing } = useAnalysis();

  const hasAnyData = Object.values(modelResults).some(
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="mb-6">
            <div className="flex items-center gap-3 mb-1">
              ...
              {isAnalysing && (
                <span className="text-[10px] px-2.5 py-1 rounded-full flex items-center gap-1.5"
                  style={{ fontFamily: "'Space Mono',monospace", background: "rgba(57,255,142,0.08)", border: "1px solid rgba(57,255,142,0.2)", color: "#39ff8e" }}>
